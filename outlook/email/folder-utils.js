...
async function getFolderIdByName(folderName) {
  try {
    ...
    const allFoldersResponse = await graphService.graphRequest(
...
async function getAllFolders() {
  try {
    ...
    const childFolderPromises = foldersWithChildren.map(async (folder) => {
      try {
        const childResponse = await graphService.graphRequest(
          `me/mailFolders/${folder.id}/childFolders?$select=id,displayName,parentFolderId,childFolderCount,totalItemCount,unreadItemCount`
