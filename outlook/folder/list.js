 ...
 */
async function getAllFoldersHierarchy(includeItemCounts) {
  try {
    ...
    const response = await graphService.graphRequest(
      `/me/mailFolders?$top=100&$select=${selectFields}`
    );
    ...
    const childFolderPromises = foldersWithChildren.map(async (folder) => {
      try {
        const childResponse = await graphService.graphRequest(
          `/me/mailFolders/${folder.id}/childFolders?$select=${selectFields}`
        );
